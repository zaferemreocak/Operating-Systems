...
void systemCommands(char *args[],int ct,int background){
	...
	while(i<ct){
		...
		if(checkPath[0] == '|'){
            pid_t saveParent;
            saveParent=fork();
            //fprintf(stderr,"number of pipe: %d",np);
...
//Executes when # of pipe operator is 1
void returnCommands(char *args[]){
    int i=0,j=0,a=0;
    char *command1[40];char *command2[40];
 while(args[a]!=NULL){

    a=pipIn+1;

    //Make pointers NULL to execute successfully
    while(i<40){
        command1[i]=NULL;
        command2[i]=NULL;
        i++;
    ...
    pipIn=a;
    //Surpass pipe operator
    i++;a++;

    //Get 2nd command
    while( (j<40) && strcmp(args[a],"|")!=0 ){
        ...
        if(args[a]==NULL){
            ...
            pipeCommandsForSinglePipe(command1,command2);
            return;
        }
        if( strcmp(args[a],"|")==0 && args[a+1] != NULL){
...
//When # of pipe operator is 1, it executes
void pipeCommandsForSinglePipe(char *command1[],char *command2[]){//(char *args[],int myct){
    ...
    /*for(i=0;i<=39;i++){
        fprintf(stderr, "\nCommand1: %s\tCommand2: %s\n", command1[i],command2[i]);
    }*/


    if( (pipe(fd) == -1) || ((childpid = fork()) == -1) ) {
        fprintf(stderr,"Failed to setup pipeline!");
        return;
    }
    //Child Command >>> i.e. LS
    if(childpid == 0){
        if(dup2(fd[1] ,STDOUT_FILENO) == -1){
            fprintf(stderr,"Failed to redirect stdout of the first command!");
        }
        else if( (close(fd[0]) == -1) || (close(fd[1]) == -1) ){
            fprintf(stderr,"Failed to close extra pipe descriptors on the first command!");
        }
        else{
            //OUTPUT=popen(command1[0],"r");
            //outputCommand=fileno(OUTPUT);
            execvp(command1[0],command1);
            fprintf(stderr,"Failed to exec the first command!");
        }
        //return;
    }
    //Parent Command >>> i.e. SORT
    if(dup2(fd[0], STDIN_FILENO) == -1){
        fprintf(stderr,"Failed to redirect stdin of sort");
    }
    else if( (close(fd[0]) == -1) || (close(fd[1]) == -1) ){
        fprintf(stderr,"Failed to close extra pipe file descriptors on the second command!");
    }
    else{
